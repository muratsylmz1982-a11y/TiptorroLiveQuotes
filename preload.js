...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    if (!document.getElementById('refreshOverlay')) {
      // Versuche Logo-Pfad vom Main-Prozess zu holen, Fallback auf SVG wenn fehlschlägt
      let logoPath = '';
      try {
        // Direkt ipcRenderer verwenden (wir sind im preload.js)
        console.log('[PRELOAD] Rufe get-logo-path IPC auf...');
        const pathFromMain = await ipcRenderer.invoke('get-logo-path');
        console.log('[PRELOAD] Antwort vom Main-Prozess:', pathFromMain);
        if (pathFromMain) {
          logoPath = pathFromMain;
          console.log('[PRELOAD] Logo-Pfad vom Main-Prozess erhalten:', logoPath);
        } else {
          console.error('[PRELOAD] Main-Prozess hat null zurückgegeben');
          throw new Error('Main-Prozess hat keinen Logo-Pfad zurückgegeben');
        }
      } catch (pathErr) {
        console.error('[PRELOAD] Fehler beim Abrufen des Logo-Pfads:', pathErr);
        console.warn('[PRELOAD] Verwende SVG-Fallback');
      ...
      overlay.innerHTML = `
        <style>
          ...
          .waiting-logo {
            ...
            filter: drop-shadow(0 2px 8px rgba(38, 153, 71, 0.5));
            /* ROTATION ANIMATION - 3D-Flip zur Seite (Schnell → Langsam → Schnell) */
          ...
          @keyframes rotateLogo {
            /* Schnell voll drehen, dann langsam, dann nochmal schnell voll drehen */
            0% {
              transform: perspective(1000px) rotateY(0deg);
            }
            30% {
              transform: perspective(1000px) rotateY(360deg);  /* Schnell eine volle Umdrehung */
            }
            /* Kurz langsam */
            35% {
              transform: perspective(1000px) rotateY(380deg);  /* Langsam weiter */
            }
            40% {
              transform: perspective(1000px) rotateY(360deg);  /* Kurz zurück */
            }
            /* Nochmal schnell voll drehen */
            70% {
              transform: perspective(1000px) rotateY(720deg);  /* Nochmal schnell eine volle Umdrehung */
            }
            /* Kurz langsam */
            75% {
              transform: perspective(1000px) rotateY(740deg);  /* Langsam weiter */
            }
            80% {
              transform: perspective(1000px) rotateY(720deg);  /* Kurz zurück */
            }
            /* Zurück zum Start für nahtlose Wiederholung (720° = 0°) */
            100% {
              transform: perspective(1000px) rotateY(720deg);  /* Bleibt bei 720° (= 0° für Wiederholung) */
            }
          }
          .waiting-status {
      ...
      // Event-Listener für Logo hinzufügen (nachdem Element erstellt wurde, CSP-konform)
      const logoImg = overlay.querySelector('.waiting-logo');
      if (logoImg) {
        logoImg.addEventListener('load', function() {
          console.log('[PRELOAD] Logo erfolgreich geladen:', this.src);
          console.log('[PRELOAD] Logo natürliche Dimensionen:', this.naturalWidth, 'x', this.naturalHeight);
          console.log('[PRELOAD] Logo sichtbare Dimensionen:', this.offsetWidth, 'x', this.offsetHeight);
          console.log('[PRELOAD] Logo complete:', this.complete);
          console.log('[PRELOAD] Logo getBoundingClientRect:', JSON.stringify(this.getBoundingClientRect()));
          if (this.naturalWidth === 0 || this.naturalHeight === 0) {
            console.error('[PRELOAD] WARNUNG: Logo hat keine Dimensionen!');
          }
        });
        
        logoImg.addEventListener('error', function() {
          console.error('[PRELOAD] Fehler beim Laden des Logos:', this.src);
          this.style.display = 'none';
        });
      }
    }
  } catch (e) {
    console.error('[PRELOAD] Fehler beim Erstellen des Overlays:', e);
  }
});
